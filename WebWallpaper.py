...
def find_workerw():
    ...
    return workerw.value or None

def embed_into_desktop(hwnd):
    """Re-parent a Qt window into the WorkerW layer behind desktop icons."""
...
class WallpaperWindow(QMainWindow):
    def __init__(self, url: str, cfg: dict):
        ...
        # Smooth scrolling (uses compositor, keeps GPU busy)
        s.setAttribute(QWebEngineSettings.WebAttribute.ScrollAnimatorEnabled, True)

        if cfg.get("mute_audio"):
    ...
    def embed(self):
        hwnd = int(self.winId())
        return embed_into_desktop(hwnd)
...
class App(QApplication):
    def __init__(self, argv):
        ...
        reload_act.triggered.connect(lambda: self.wallpaper.view.reload())
